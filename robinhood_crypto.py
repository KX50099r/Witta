...
API_KEY = os.environ['RHC_API_KEY']
BASE64_PRIVATE_KEY =  os.environ['RHC_PRI_KEY']
...
def main():
    ...
    if not os.environ.get('RHC_API_KEY'):
        print("Error: RHC_API_KEY environment variable is not set")
        return
    if not os.environ.get('RHC_PRI_KEY'):
        print("Error: RHC_PRI_KEY environment variable is not set")
        return
        
    api_trading_client = CryptoAPITrading()
    response = api_trading_client.get_account()
    if response is None:
        print("Error: Failed to get account information. Check your API credentials.")
    else:
